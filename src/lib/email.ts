// Email sending via EmailJS (server-side)
...
async function send(to: string, subject: string, html: string) {
  if (!PRIVATE_KEY) {
    console.warn('EMAILJS_PRIVATE_KEY not set — skipping email')
    return { success: false }
  }
  try {
    await emailjs.send(
      SERVICE_ID, TEMPLATE_ID,
      { to_email: to, to_name: '', subject, message: html },
...
const welcomeHtml = (name: string, points: number) => base(`
  ...
  <p style="margin:0 0 24px;color:#44403c;font-size:15px;line-height:1.7;">Your <strong>Nelliy's Rewards</strong> account is all set!</p>
  ...
  <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
    <a href="https://nelliy.com/dashboard" target="_blank" style="display:inline-block;background:linear-gradient(135deg,#f59e0b,#ea580c);color:#fff;text-decoration:none;font-size:15px;font-weight:700;padding:14px 36px;border-radius:12px;">☕ Go to My Dashboard</a>
  </td></tr></table>`)
...
const rewardHtml = (reward: string, code: string) => base(`
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:linear-gradient(135deg,#fffbeb,#fef3c7);border:2px solid #fcd34d;border-radius:16px;margin-bottom:24px;">
    <tr><td style="padding:24px;text-align:center;">
      <p style="margin:0 0 8px;color:#92400e;font-size:12px;font-weight:600;text-transform:uppercase;">Your Code</p>
